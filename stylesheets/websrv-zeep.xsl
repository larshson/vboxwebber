...
<!--
    websrv-zeep.xsl:
        XSLT stylesheet that generates VirtualBox service methods from
        VirtualBox.xidl.

        This is a heavily modified version of websrv-python.xsl, in order
        to get webservice support for Python 3 with help from the zeep
        library. The wrappers can be used directly without the need of
        intalling the VirtualBox SDK.

        The IWebsessionManager2 takes a wsdl input parameter, for newer
        web services this can be fetched by appending "?wsdl" to the url
        parameter but for older versions a local wsdl file has to be 
        supplied, on the "file:///path/to/wsdl-file" format.

        This stylesheet also generates a lot of documentation for interfaces,
        methods, enums, parameters etc. in order to assist python developers
...
<xsl:template name="emitGetAttribute">
  ...
  <xsl:variable name="fname"><xsl:call-template name="makeGetterName"><xsl:with-param name="attrname" select="$attrname"/></xsl:call-template> </xsl:variable>
   def <xsl:value-of select="$fname"/>(self):&#10;       """<xsl:apply-templates select="$desc"/> """
       val = self.mgr.getService().<xsl:value-of select="$ifname"/>_<xsl:value-of select="$fname"/>(self.handle)
...
<xsl:template name="emitSetAttribute">
  ...
  <xsl:variable name="fname"><xsl:call-template name="makeSetterName"><xsl:with-param name="attrname" select="$attrname"/></xsl:call-template> </xsl:variable>
   def <xsl:value-of select="$fname"/>(self, value):&#10;       """<xsl:apply-templates select="$desc"/> """
       _this=self.handle
       if type(value) in [int, bool, str<xsl:if test="$attrsafearray='yes'">, tuple, list</xsl:if>]:
            _val = value
       else:
            _val = value.handle
       self.mgr.getService().<xsl:value-of select="$ifname"/>_<xsl:value-of select="$fname"/>(_this, _val)
</xsl:template>
...
<xsl:template name="genreq">
  <!-- IWebsessionManager seems to have the only interface methods that 
       does not need the "this" parameter -->
  <xsl:choose>
    <xsl:when test="../@name='IWebsessionManager'">
       req = ()</xsl:when>
    <xsl:otherwise>
       req = (self.handle,)</xsl:otherwise></xsl:choose><xsl:for-each select="param[@dir='in']">
       req += (<xsl:call-template name="convertInParam">
           <xsl:with-param name="type" select="@type" />
           <xsl:with-param name="safearray" select="@safearray" />
           <xsl:with-param name="arg" select="concat('_arg_', @name)" />
         </xsl:call-template>,)</xsl:for-each>
       val = self.mgr.getService().<xsl:value-of select="../@name"/>_<xsl:value-of select="@name"/>(*req)
           ...
           <xsl:with-param name="value">
             <xsl:choose>
               <xsl:when test="../param[@dir='out']">
                 <!-- multiple return parameters, the 'return' one is contained in the dict["returnval"] -->
                 <xsl:value-of select="'val[&quot;returnval&quot;]'"/>
               </xsl:when>  
               <xsl:otherwise>
       ...
       <xsl:for-each select="param[@dir='out']">
         ...
         <xsl:call-template name="emitOutParam">
           ...
           <xsl:with-param name="methodname" select="@name" />
           <xsl:with-param name="type" select="@type" />
           <xsl:with-param name="value" select="concat('val[&quot;',@name,'&quot;]')" />
           <xsl:with-param name="safearray" select="@safearray"/>
         </xsl:call-template>
       </xsl:for-each>
       <xsl:text>&#10;</xsl:text>
...
<xsl:template match="desc/result"/>
...
<xsl:template match="method/desc">
  ...
  <xsl:for-each select="../param">
    <xsl:call-template name="docparam"/>
  </xsl:for-each>
...
-->
<xsl:template name="docparam">
  <xsl:if test="@dir='in'">
    <xsl:text>        :param </xsl:text>
    <xsl:value-of select="concat('_arg_', @name)"/>
    <xsl:text>: </xsl:text>
    <xsl:value-of select="normalize-space(desc/text() | *[not(self::note or self::see)])"/>
  </xsl:if>
  <xsl:if test="@dir='return'">
    ...
    <xsl:value-of select="normalize-space(@type | *[not(self::note or self::see)])"/>
    <xsl:if test="@safearray='yes'">
      <xsl:text>]</xsl:text>
    </xsl:if>
  </xsl:if>
...
<xsl:template match="enum/desc">
  ...
  <xsl:for-each select="../const">
    <xsl:value-of select="concat('&quot;', @name, '&quot;')"/> = <xsl:value-of select="@value"/>
    <xsl:text>:</xsl:text>
    <xsl:choose>
        <xsl:when test="not(contains(desc,'&#x0a;'))">
          <xsl:text>&#x0a;        </xsl:text>
          <xsl:apply-templates select="desc"/>
          <xsl:text>&#x0a;      </xsl:text>
        </xsl:when>
...
class <xsl:value-of select="@name"/>:
   ...
   _ValueMap = {<xsl:for-each select="const">'<xsl:value-of select="@name"/>':<xsl:value-of select="@value"/><xsl:if test="not(position()=last())">,</xsl:if>
              </xsl:for-each>}
...
<!-- The following templates removes double newlines in the output -->
